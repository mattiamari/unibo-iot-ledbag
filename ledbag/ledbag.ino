...
const unsigned int DEBOUNCE_INTERVAL = 100;
...
void state_welcome() {
  ...
  if (level_timeout != new_timeout) {
    ...
    sprintf(msg_buff, "Selected level %d. Timeout is now %d ms", level+1, level_timeout);
...
void bag_success() {
  ...
  sprintf(msg_buff, "Hurry up! Timeout is in %d ms", level_timeout);
  Serial.println(msg_buff);
